...
-- Nombre: Christian Velazquez |  Fecha: 19/07/2026
-- ======================================
...
-- Paso 2 — Cargar las 16 parejas libro-autor
-- Cada fila dice "este libro fue escrito por este autor". El author_order indica si es autor principal (1) o co-autor (2).
...
-- Paso 3 — Verifica la N:M
-- Comprueba que la relación funciona en la dirección difícil: ¿qué libros tienen más de un autor?
...
-- Fase 6 — Préstamo y devolución como transacciones (15 min)
-- 6.1 — Registrar un préstamo (Mary pide Deep Learning)
-- Mary (usuario 3) quiere llevarse Deep Learning (libro 12). 
-- Es una operación de dos mitades: bajar el stock y registrar el préstamo. Las dos tienen que pasar juntas → transacción.
-- Paso 1 — Abrir la transacción y verificar que hay stock
/* START TRANSACTION;

SELECT id, title, stock FROM books WHERE id = 12 AND stock > 0;

-- Paso 3 — Registrar el préstamo (devolución esperada en 14 días)
INSERT INTO loans
    (user_id, book_id, loan_date, due_date)
VALUES
    (3, 12, CURDATE(), DATE_ADD(CURDATE(), INTERVAL 14 DAY));
    
-- Paso 4 — Verificar las dos mitades antes de cerrar
SELECT id, title, stock FROM books WHERE id = 12;
SELECT * FROM loans WHERE id = LAST_INSERT_ID();

-- Paso 5 — Cerrar la operación (COMMIT)
COMMIT;

-- 6.2 — Procesar una devolución con multa (Alice devuelve el libro 4)
-- El préstamo id = 6 (Alice, "A Brief History of Time") está 6 días vencido. Se devuelve hoy → multa de $2.50/día. La devolución es la operación inversa al préstamo: marcar devuelto + calcular multa + sumar stock. También en una transacción, también en la misma pestaña, bloque por bloque.

-- Paso 1 — Abrir la transacción y calcular la multa
START TRANSACTION;
...
-- Paso 2 — Marcar el préstamo como devuelto y aplicar la multa
UPDATE loans
SET return_date = CURDATE(),
    fine = @fine
WHERE id = 6;

-- Paso 3 — Devolver el ejemplar al stock
UPDATE books
SET stock = stock + 1
...
/* -- Fase 7 — Probar la integridad referencial sin destruir tus datos
- 7.1 — ON DELETE SET NULL (en books.category_id)
-- La categoría 3 es "History". El único libro de esa categoría es 21 Lessons for the 21st Century (libro 7). 
...
-- 7.2 — ON DELETE RESTRICT (en loans.user_id)
...
-- Estos INSERT deben fallar. Como fallan, no insertan nada — no hay que deshacer:
...
-- Trigger de auditoría de precios (5 pts)
-- Trigger es tema de Week 10. Si lo intentas ahora, no te frustres si no entiendes toda la sintaxis — 
-- vuelve a esto después de Week 10.
